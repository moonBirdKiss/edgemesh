...
func (t *EdgeTunnel) GetRouteStream(opts RouteProxyOptions) (*StreamConn, error) {
	...
	if !exists {
		...
		klog.Infof("[GetRouteStream]: Could not find peer %s in cache, auto generate peer info: %s", destName, destInfo)
	...
	if err = AddCircuitAddrsToPeer(&destInfo, t.relayMap); err != nil {
		return nil, fmt.Errorf("[GetRouteStream]: failed to add circuit addrs to peer %s", destInfo)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("[GetRouteStream]: new stream between %s: %s err: %w", destName, destInfo, err)
	}
	klog.Infof("[GetRouteStream]: New stream between peer %s: %s success", destName, destInfo)
	...
	if err = streamWriter.WriteMsg(msg); err != nil {
		...
		if resetErr != nil {
			return nil, fmt.Errorf("[GetRouteStream]: stream between %s reset err: %w", destName, resetErr)
		}
		return nil, fmt.Errorf("[GetRouteStream]: write conn msg to %s err: %w", destName, err)
	}
	...
	if err = streamReader.ReadMsg(msg); err != nil {
		...
		return nil, fmt.Errorf("[GetRouteStream]: read conn result msg from %s err: %w", destName, err)
	}
	...
	klog.Infof("[GetRouteStream]: Success proxy for {%s %s %s %s}", opts.Protocol, destName, opts.NodeName, opts.Port)
...
func (t *EdgeTunnel) routeStreamHandler(stream network.Stream) {
	...
	// 尝试在这里读取 stream 中的数据，然后保存在buffer中
	klog.Infof("[route]: Try to read data from stream")
	...
	for {
		...
		if err != nil {
			if err == io.EOF {
				klog.Infof("[route]: Read data from stream EOF")
				break
			} else {
				klog.Infof("[route]: Read data from stream error: ", err)
				break
			}
		}
		// 将读取的数据写入到缓冲区
	...
	klog.Infof("[route]: Read data successfully: %s", buffer.String())

	// 尝试在这里写入数据到 stream 中
	response := "HTTP/1.1 200 OK\r\nContent-Length: 8\r\n\r\nReceived"
	_, err = fmt.Fprintf(stream, "%s", response)
	if err != nil {
		klog.Errorf("[route]: Write data to stream error: ", err)
		return
	}
	err = stream.Close()
	if err != nil {
		klog.Errorf("[route]: Close stream error: ", err)
		return
	}
	...
	// 这里本来是要进行多跳处理的，但是由于目前的实现是，所以直接调用 GetProxyConn
	if len(path) != 0 {
	...
	case TCP:
		go func() {
			_, err := io.Copy(proxyConn, &buffer)
			if err != nil {
				panic(err)
			}
